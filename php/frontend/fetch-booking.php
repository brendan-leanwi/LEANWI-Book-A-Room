        ...
        if ($results[0]['venue_id'] != $venue_id) {
            ...
            $safe_results = array_map(function($result) {
                return [
                    ...
                    'name' => html_entity_decode($result['name'], ENT_QUOTES),
                    //'organization' => esc_html($result['organization']),
                    'organization' => html_entity_decode($result['organization'], ENT_QUOTES),
                    'email' => sanitize_email($result['email']),
                    'phone' => esc_html($result['phone']),
                    //'physical_address' => esc_html($result['physical_address']),
                    'physical_address' => html_entity_decode($result['physical_address'], ENT_QUOTES),
                    ...
                    //'booking_notes' => esc_textarea($result['booking_notes']),
                    'booking_notes' => html_entity_decode($result['booking_notes'], ENT_QUOTES),
